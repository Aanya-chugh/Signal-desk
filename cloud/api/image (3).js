...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (gem) return res.status(200).json({ niche, source: "gemini-2.5-flash-image", image: gem });
  } catch { /* fall through to Flux */ }
  ...
  if (fx.image) return res.status(200).json({ niche, source: "pollinations-flux", image: fx.image });
  return res.status(502).json({ error: fx.error || "image generation failed" });
}

// Google Gemini 2.5 Flash Image — returns a base64 data URL, or null to trigger the fallback.
