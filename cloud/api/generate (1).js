  ...
  try {
    ...
    if (mode === "script") {
      ...
      prompt =
        ...
        `Newsjack this story to promote ${niche} to a US audience — natural and tasteful, never forced or exploitative.${sens}\n` +
        `Include a one-line description of the ideal main image to pair with the ad.\n` +
        `Return ONLY valid JSON, no markdown. Every value a plain string; points is an array of exactly 3 short strings:\n` +
        `{"mainImage":"one-line description of the ideal ad photo/visual","headline":"<=8 words, punchy, may begin with one emoji","body":"2-3 short sentences of primary ad text","points":["short benefit","short benefit","short benefit"],"cta":"button label, e.g. Learn More or Get a Quote"}`;
      max_tokens = 420;
    } else {
      prompt =
