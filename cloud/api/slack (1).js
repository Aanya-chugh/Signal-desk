...
export default async function handler(req, res) {
  ...
  const niche = pickNiche(payload, action.block_id, meta.n || "Home Insurance");
  ...
  try {
    if (kind === "gen_copy") {
      ...
      // generate coherent copy + image prompt together so the photo matches a headline,
      // then post the clean (text-free) photo with the matching headline + description.
      const c = await generate({ mode: "imagead", niche, article });
      if (c.error) throw new Error(c.error);
      const img = await makeImage(niche, str(c.imgprompt));
      const d = dataUrlToBuffer(img.image);
      if (!d) throw new Error(img.error || "image failed");
      const caption =
        `🖼 *Image — ${niche}*\n` +
        `*${str(c.headline)}*\n${str(c.subhead)}`;
      await uploadBytes(channel, thread, d.bytes, d.mime, "signal-desk-image.png", caption);
