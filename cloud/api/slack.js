...
export default async function handler(req, res) {
  ...
  try {
    if (kind === "gen_copy") {
      ...
      const img = await makeImage(niche, str(pr.prompt));
      const d = dataUrlToBuffer(img.image);
      if (img.error || !d) throw new Error(img.error || "image failed");
      await uploadBytes(channel, thread, d.bytes, d.mime, "signal-desk-image.png", `🖼 *Ad image — ${niche}*`);
