...
//   📸 Image Ad -> a complete, UNIQUE Facebook-style ad creative generated by Nano Banana Pro
//                  (headline rendered in-image, varied design each time — no template, no branding)
...
const clean = (v) => str(v).replace(/\*+/g, "").trim();              // strip markdown bold
const firstSentence = (v) => clean(v).split(/(?<=[.!?])\s/)[0].slice(0, 120);
...
async function makeImage(niche, prompt, tier) {
  ...
  if (tier) body.tier = tier;                 // "pro" routes to Nano Banana Pro for ads
...
/* ---------- Testimonial / social-proof AD creative (Nano Banana Pro) ---------- */
// Rotating testimonial layouts so each ad looks fresh — but always the high-performing,
// native "real customer + 5 stars + quote" social-proof style.
const AD_STYLES = [
  "a photo card on top showing the authentic customer, with a clean white testimonial card below it holding a 5-star rating, the short quote, and the customer's name; the bold headline sits above or beside the testimonial",
  "a full-bleed authentic candid photo with a tidy testimonial caption card overlaid near the bottom (5 gold stars, the short quote, the name), and the headline set bold in a clear area of the photo",
  "a side-by-side split: an authentic candid customer photo on one side and a soft-colored panel on the other holding the headline, a 5-star rating, the quote, and the name",
  "a native social-feed look: an authentic photo with a small rounded testimonial bubble (5 stars + short quote + name) and the headline as a clean caption, as if screenshotted from a real feed",
  "a warm lifestyle photo with a clean lower-third testimonial bar across the bottom: 5 stars, the concise quote and the customer name, with the headline in confident modern type above",
];
function adPrompt(niche, headline, subline, story, style) {
  return [
    "Create a high-end, scroll-stopping Facebook / Instagram TESTIMONIAL advertisement, 1:1 square,",
    "that feels authentic and native in-feed (user-generated-content social-proof style), with the polish of a top DTC brand's best-performing ad. Real, candid, believable — NOT a staged studio ad and NOT a generic template.",
    "Layout for THIS specific ad: " + style + ".",
    "Show an authentic, candid, real-looking happy customer relevant to the offer and the mood below.",
    "Include a clear 5-star rating and a SHORT, natural, first-person customer testimonial quote (about 8–16 words) with a first name and last initial.",
    "Make the quote specific and believable, and DIFFERENT every time — do NOT reuse a generic line like 'Saved me when I needed it most'.",
    "The product / offer being advertised: " + niche + ".",
    "Topical mood (informs the scene, do NOT print this as text): " + story + ".",
    'Feature this headline as the hook, large and perfectly spelled: "' + headline + '".',
    subline ? 'Include this supporting line, smaller and secondary: "' + subline + '".' : "",
    "All typography must be crisp, correctly spelled, and cleanly laid out.",
    "Strictly NO brand names, NO logos, NO watermark, NO 'Signal Desk', and NO fake clickable UI buttons.",
...
/* ---------- generate one Facebook-style ad image (Pro) ---------- */
async function makeAdImage(niche, article) {
  const cp = await generate({ mode: "copy", niche, article });
  const headline = clean(cp.headline) || (niche + " — A Smarter Choice");
  const subline = firstSentence(cp.description);
  const prompt = adPrompt(niche, headline, subline, str(article.title), pickStyle());
  const img = await makeImage(niche, prompt, "pro");
  return { img, headline, description: clean(cp.description) };
}
...
export default async function handler(req, res) {
  ...
  try {
    if (kind === "gen_copy") {
      ...
      const { img, headline, description } = await makeAdImage(niche, article);
      const d = dataUrlToBuffer(img.image);
      if (!d) throw new Error(img.error || "ad image failed");
      await uploadBytes(channel, thread, d.bytes, d.mime, "signal-desk-ad.png",
      ...
      // 3) Facebook-style ad creative (Nano Banana Pro)
      const headline = clean(cp.headline) || (niche + " — A Smarter Choice");
      const subline = firstSentence(cp.description);
      const adImg = await makeImage(niche, adPrompt(niche, headline, subline, str(article.title), pickStyle()), "pro");
      const ad = dataUrlToBuffer(adImg.image);
      if (ad) await uploadBytes(channel, thread, ad.bytes, ad.mime, "signal-desk-ad.png", `📸 *Image Ad — ${niche}*\n*${headline}*`);
