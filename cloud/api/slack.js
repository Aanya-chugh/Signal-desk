...
export default async function handler(req, res) {
  ...
  try {
    if (kind === "gen_copy") {
      ...
      // Generate all four assets for THIS story+niche. One news-anchored photo is made once and
      // reused for both the clean image and the poster. The text generations run in parallel to
      // stay inside the time budget.
      const safe = (p) => generate(p).catch(() => ({ error: "failed" }));
      const [cp, ad, scr, prompt] = await Promise.all([
        safe({ mode: "copy", niche, article }),
        safe({ mode: "imagead", niche, article }),
      ...
      // 2) Clean image + matching copy
      if (d) {
        let cap = `🖼 *Image — ${niche}*`;
        if (!cp.error) cap += `\n*${str(cp.headline)}*\n${str(cp.description)}`;
        await uploadBytes(channel, thread, d.bytes, d.mime, "signal-desk-image.png", cap);
      }
      // 3) Image-ad poster (same photo, copy burned in)
      const poster = await buildPoster({
        niche, badge: str(ad.badge), headline: str(ad.headline),
        subhead: str(ad.subhead), cta: str(ad.cta), imageBuf: d ? d.bytes : null,
      });
      await uploadBytes(channel, thread, poster, "image/png", "signal-desk-poster.png", `📸 *Image Ad — ${niche}*`);
      // 4) Video script
      const sc = (scr && scr.script) || {};
      let txt = `🎬 *Video Ad — ${niche}*\n_${str(sc.concept)}_`;
      (sc.scenes || []).forEach((x) => {
        txt += `\n\n*${str(x.scene)}*  ${str(x.time)}\n• *Visual:* ${str(x.visual)}\n• *VO:* ${str(x.voiceover)}\n• *On-screen:* ${str(x.onscreen)}`;
      });
