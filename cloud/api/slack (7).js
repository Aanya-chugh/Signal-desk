...
function benefitOnly(desc) {
  // Split only on commas, em-dashes, or a spaced hyphen used as a dash — NOT hyphens inside words (e.g. "walk-in").
  const parts = firstSentence(desc).split(/\s*—\s*|\s*,\s*|\s+-\s+/);
...
function adPrompt(niche, headline, subline, scene, style) {
  return [
    ...
    "Build the photographic scene on this news-anchored description so the ad clearly references the current story (keep it positive and ad-appropriate): " + scene + ".",
    "It advertises " + niche + ".",
...
// ONE combined call writes the headline, the description, AND a matching scene together — so the
// image is purpose-built to fit BOTH the news headline and the ad description, framed for the niche.
async function adSet(niche, article) {
  const r = await generate({ mode: "adset", niche, article });
  const headline = clean(r.headline) || "A Smarter Choice";
  const description = clean(r.description);
  let scene = str(r.scene).trim();
  if (!scene) scene = `a scene that clearly reflects this news — "${str(article.title)}" — in a positive, ad-ready setting tied to ${niche}`;
  return { headline, description, subline: benefitOnly(description), scene };
}
async function makeOneAd(niche, headline, subline, scene, style) {
  return makeImage(niche, adPrompt(niche, headline, subline, scene, style), "pro");
}
async function makeAdImage(niche, article) {     // single ad (used by ⚡ Generate all)
  const set = await adSet(niche, article);
  const img = await makeOneAd(niche, set.headline, set.subline, set.scene, pickStyle());
  return { img, headline: set.headline, description: set.description };
}
...
export default async function handler(req, res) {
  ...
  try {
    if (kind === "gen_copy") {
      ...
      const { headline, description, subline, scene } = await adSet(niche, article);
      const styles = pickStyles(2);
      const results = await Promise.all(
        styles.map((s) => makeOneAd(niche, headline, subline, scene, s).catch((e) => ({ error: String((e && e.message) || e) })))
      );
      let n = 0;
