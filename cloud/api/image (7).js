...
// Returned as a base64 data URL. Admin-only.
// Body: { niche, prompt, tier? }  — tier:"pro" uses Nano Banana Pro (best in-image text, for ad creatives).
// Env: AUTH_SECRET (req), GEMINI_API_KEY (org paid key), GEMINI_IMAGE_MODEL (optional, default
//   gemini-2.5-flash-image = Nano Banana), GEMINI_PRO_MODEL (optional, default gemini-3-pro-image-preview
//   = Nano Banana Pro), POLLINATIONS_KEY (req), IMAGE_MODEL (optional Pollinations primary).
...
const GEMINI_PRO_MODEL = process.env.GEMINI_PRO_MODEL || "gemini-3-pro-image-preview"; // Nano Banana Pro
...
export default async function handler(req, res) {
  ...
  // The ad path (pro) sends a full, self-contained ad prompt and must NOT have "no text" appended.
  let prompt;
  if (isPro && typeof body.prompt === "string" && body.prompt.trim()) {
    prompt = body.prompt.trim();
  } else {
    let scene = typeof body.prompt === "string" ? body.prompt.trim() : "";
    if (!scene) scene = SCENES[niche] || SCENES["Home Insurance"];
    prompt =
  ...
  const model = isPro ? GEMINI_PRO_MODEL : GEMINI_MODEL;
  // Pro renders crisp in-image text → ask for a square 2K image. Regular stays on the simple body.
  const genConfig = isPro ? { responseModalities: ["IMAGE"], imageConfig: { aspectRatio: "1:1", imageSize: "2K" } } : null;

  // 1) Gemini / Nano Banana (best, org paid key).
  const gem = await geminiImage(prompt, model, genConfig);
  if (gem.image) return res.status(200).json({ niche, source: "gemini:" + model, image: gem.image });
...
async function geminiImage(prompt, model, genConfig) {
  ...
  try {
    ...
    if (genConfig) reqBody.generationConfig = genConfig;
    const r = await fetch(
    ...
    if (!r.ok) {
      ...
      return { error: model + " HTTP " + r.status + ": " + (t || "").slice(0, 180) };
    }
    ...
    return { error: model + " returned no image" + (said ? " (said: " + said + ")" : "") };
  } catch (e) {
    clearTimeout(timer);
    return { error: model + " request failed: " + String((e && e.message) || e).slice(0, 160) };
  }
}
