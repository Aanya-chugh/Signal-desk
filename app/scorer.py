...
# IMPORTANT: use distinctive multi-word phrases, not bare ambiguous words. A bare
# "memory" matches computer-memory stories, "windows" matches Microsoft Windows,
# "focus" matches "focus on", "policy" matches political policy, etc. Precision
# beats recall here: a wrong tag is worse than "General".
NICHES = {
    "Home Insurance": {
        "home insurance", "homeowners insurance", "homeowner", "homeowners",
        "flood insurance", "homeowners policy", "insurance premium", "insurance claim",
        "wildfire", "wildfires", "hurricane", "hurricanes", "hailstorm",
        "tornado", "tornadoes", "natural disaster", "storm damage", "property damage",
        "home insurer", "flood damage",
    },
    "Auto Insurance": {
        "auto insurance", "car insurance", "vehicle insurance", "car crash",
        "multi-car", "pile-up", "collision", "fender bender", "dui",
        "totaled", "car recall", "vehicle recall", "auto recall", "drunk driving",
    },
    "Medicare": {
        "medicare", "medicaid", "medicare advantage", "social security",
        "open enrollment", "part b", "part d", "obamacare",
        "prescription drug", "drug prices", "retirement benefits", "retiree health",
    },
    "Refinance": {
        "refinance", "refi", "mortgage", "mortgages", "mortgage rate",
        "mortgage rates", "interest rate", "interest rates", "rate cut",
        "federal reserve", "home loan", "heloc", "home equity",
        "fixed-rate", "housing market", "home prices", "mortgage lender",
    },
    "Weight Loss Supplements": {
        "weight loss", "weight-loss", "ozempic", "wegovy", "semaglutide",
        "obesity", "diet pill", "appetite suppressant", "fat loss",
        "overweight", "shedding pounds", "metabolism boost",
    },
    "Memory Loss Supplements": {
        "memory loss", "memory care", "alzheimer", "alzheimer's", "dementia",
        "cognitive decline", "brain health", "brain fog", "nootropic",
        "mental decline", "forgetfulness", "cognitive function",
    },
    "Gun Permits": {
        "concealed carry", "gun permit", "gun permits", "carry permit",
        "second amendment", "firearm", "firearms", "handgun", "ccw",
        "gun law", "gun laws", "gun control", "gun rights", "background check", "nra",
    },
    "Bathroom Services": {
        "bathroom remodel", "bathroom renovation", "bathroom upgrade",
        "walk-in shower", "walk-in tub", "shower remodel", "shower installation",
        "bathtub", "grab bar", "wet room",
    },
    "Window Services": {
        "replacement window", "replacement windows", "window replacement",
        "new windows", "energy-efficient windows", "double-pane", "vinyl windows",
        "drafty windows", "window installation", "storm windows",
    },
    "Bizops": {
        "side hustle", "side hustles", "work from home", "work-from-home",
        "business opportunity", "passive income", "side income",
        "make money online", "be your own boss", "start a business",
        "self-employed", "gig economy", "remote work",
    },
}
...
def score_article(title: str, summary: str = "", domain: str = "",
    ...
    # A story can genuinely bridge to several niches, so we keep ALL niches that
    # actually have a keyword hit (strongest first, capped at 3 tags). No hits at
    # all -> "General (bridge needed)" so we never force a weak/irrelevant match.
    niche_counts = {}
    for niche, kws in NICHES.items():
        niche_counts[niche] = _hits(ttoks, kws)
    relevant = sorted([(n, c) for n, c in niche_counts.items() if c > 0],
                      key=lambda x: x[1], reverse=True)
    matched_list = [n for n, c in relevant[:3]]
    best_n = relevant[0][1] if relevant else 0
    niche_fit = round(100 * _sat(best_n, k=1.0), 0) if best_n else 0
    matched_niche = ", ".join(matched_list) if matched_list else "General (bridge needed)"
